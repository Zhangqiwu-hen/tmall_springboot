...
public class ProductImageController {
    ...
    public List<ProductImage> list(@RequestParam("type") String type, @PathVariable("pid") int pid) throws Exception {
        ...
        if (ProductImageService.type_single.equals(type)) {
            details = this.productImageService.listSingleProductImages(product);
            return details;
        } else if (ProductImageService.type_detail.equals(type)) {
            details = this.productImageService.listDetailProductImages(product);
            return details;
        } else {
    ...
    public Object add(@RequestParam("type") String type, @RequestParam("pid") int pid, MultipartFile image, HttpServletRequest request) throws Exception {
        ...
        productImage.setProduct(this.productService.get(pid));
        this.productImageService.add(productImage);
        this.saveImageFile(productImage, image, request);
        return productImage;
    }
    ...
    public String delete(@PathVariable("id") int id) throws Exception {
        String type = this.productImageService.get(id).getType();
