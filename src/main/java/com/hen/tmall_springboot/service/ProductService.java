...
public class ProductService {
    ...
    public void add(Product bean) {
        ...
        this.productESDAO.save(bean);
    }

    @CacheEvict(allEntries = true)
    public void delete(int id) {
        this.productDAO.deleteById(id);
        this.productESDAO.deleteById(id);
    }
    ...
    public void fill(Category category) {
        ...
        this.productImageService.setFirstProductImages(products);
    ...
    public void setSaleAndReviewNumber(Product product) {
        ...
        int reviewCount = this.reviewService.getCount(product);
    ...
    public List<Product> search(String keyword, int start, int size) {
        ...
        Page<Product> page = this.productESDAO.search(queryBuilder.build());
        return page.getContent();
    }

    private void initDatabase2ES() {
        Pageable pageable = PageRequest.of(0, 10);
        Page<Product> page = this.productESDAO.findAll(pageable);
        if (page.getContent().isEmpty()) {
            List<Product> products = this.productDAO.findAll();
            for (Product product : products) {
                this.productESDAO.save(product);
            }
        }
